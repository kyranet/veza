  ...
  *process(buffer: Buffer) {
  ...
  _readMessage(body: Buffer, type: string) {
    ...
    if (type === "FLOAT32_ARRAY")
      return new Float32Array(JSON.parse(bodyString));
    if (type === "FLOAT64_ARRAY")
      return new Float64Array(JSON.parse(bodyString));
    ...
    if (type === "UINT8_CLAMPEDARRAY")
      return new Uint8ClampedArray(JSON.parse(bodyString));
